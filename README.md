<div align="center">

# 🛒 Güzel Teknoloji E-Ticaret

### Modern ve Şık Teknoloji Mağazası Prototipi

[![React](https://img.shields.io/badge/React-19-61DAFB?style=for-the-badge&logo=react&logoColor=white)](https://react.dev/)
[![TypeScript](https://img.shields.io/badge/TypeScript-5.0-3178C6?style=for-the-badge&logo=typescript&logoColor=white)](https://www.typescriptlang.org/)
[![Vite](https://img.shields.io/badge/Vite-6.2-646CFF?style=for-the-badge&logo=vite&logoColor=white)](https://vitejs.dev/)
[![Tailwind CSS](https://img.shields.io/badge/Tailwind-4.1-06B6D4?style=for-the-badge&logo=tailwindcss&logoColor=white)](https://tailwindcss.com/)

<img src="https://raw.githubusercontent.com/lucide-icons/lucide/main/icons/shopping-bag.svg" width="120" height="120" alt="Logo">

[🇹🇷 Türkçe](#-türkçe) • [🇬🇧 English](#-english)

</div>

---

## 🇹🇷 Türkçe

### ✨ Özellikler

| Özellik | Açıklama |
|---------|----------|
| 🎨 **Modern Tasarım** | Şık ve kullanıcı dostu arayüz |
| 🌙 **Karanlık Mod** | Göz yormayan dark theme desteği |
| 🛒 **Sepet Yönetimi** | LocalStorage ile kalıcı sepet |
| ❤️ **Favoriler** | Beğendiğiniz ürünleri kaydedin |
| 🔍 **Gelişmiş Filtreleme** | Kategori, marka, fiyat, renk filtreleri |
| 📱 **Responsive** | Tüm cihazlarda mükemmel görünüm |
| ⚡ **Hızlı** | Vite ile süper hızlı geliştirme |
| 🎭 **Animasyonlar** | Framer Motion ile akıcı geçişler |

### 📦 Kurulum

```bash
# Repoyu klonlayın
git clone https://github.com/kullaniciadi/guzel-teknoloji-ecommerce.git

# Klasöre gidin
cd guzel-teknoloji-ecommerce

# Bağımlılıkları yükleyin
npm install

# Geliştirme sunucusunu başlatın
npm run dev
```

### 🚀 Komutlar

| Komut | Açıklama |
|-------|----------|
| `npm run dev` | Geliştirme sunucusunu başlatır (port 3000) |
| `npm run build` | Projeyi production için derler |
| `npm run preview` | Production build'i önizler |
| `npm run lint` | TypeScript tip kontrolü yapar |

### 📁 Proje Yapısı

```
src/
├── components/          # Yeniden kullanılabilir bileşenler
│   ├── HeroSlider.tsx   # Ana sayfa slider
│   ├── Layout.tsx       # Sayfa düzeni
│   └── ProductCard.tsx  # Ürün kartı
├── pages/               # Sayfa bileşenleri
│   ├── Home.tsx         # Ana sayfa
│   ├── ProductDetail.tsx # Ürün detay
│   ├── Checkout.tsx     # Ödeme sayfası
│   ├── Brands.tsx       # Markalar
│   └── FAQ.tsx          # Sıkça sorulan sorular
├── App.tsx              # Ana uygulama
├── constants.ts         # Sabit veriler
├── types.ts             # TypeScript tipleri
└── index.css            # Global stiller
```

---

## 🇬🇧 English

### ✨ Features

| Feature | Description |
|---------|-------------|
| 🎨 **Modern Design** | Sleek and user-friendly interface |
| 🌙 **Dark Mode** | Eye-friendly dark theme support |
| 🛒 **Cart Management** | Persistent cart with LocalStorage |
| ❤️ **Favorites** | Save your favorite products |
| 🔍 **Advanced Filtering** | Category, brand, price, color filters |
| 📱 **Responsive** | Perfect display on all devices |
| ⚡ **Fast** | Super fast development with Vite |
| 🎭 **Animations** | Smooth transitions with Framer Motion |

### 📦 Installation

```bash
# Clone the repository
git clone https://github.com/username/guzel-teknoloji-ecommerce.git

# Navigate to the folder
cd guzel-teknoloji-ecommerce

# Install dependencies
npm install

# Start the development server
npm run dev
```

### 🚀 Commands

| Command | Description |
|---------|-------------|
| `npm run dev` | Starts development server (port 3000) |
| `npm run build` | Builds the project for production |
| `npm run preview` | Previews the production build |
| `npm run lint` | Runs TypeScript type checking |

### 🛠️ Tech Stack

<div align="center">

| Technology | Purpose |
|------------|---------|
| **React 19** | UI Framework |
| **TypeScript** | Type Safety |
| **Vite** | Build Tool |
| **Tailwind CSS** | Styling |
| **Framer Motion** | Animations |
| **Lucide React** | Icons |

</div>

---

<div align="center">

### 📸 Screenshots / Ekran Görüntüleri

#### 🏠 Ana Sayfa / Home Page
<img src="screenshots/home.png" width="800" alt="Ana Sayfa">

#### 🛍️ Ürün Detay / Product Detail
<img src="screenshots/product-detail.png" width="800" alt="Ürün Detay">

#### 🛒 Sepet / Cart
<img src="screenshots/cart.png" width="800" alt="Sepet">

#### 🌙 Karanlık Mod / Dark Mode
<img src="screenshots/dark-mode.png" width="800" alt="Karanlık Mod">

---

### 📄 License / Lisans

MIT License - Dilediğiniz gibi kullanabilirsiniz! / Feel free to use as you wish!

---

<p>
  Made with ❤️ by <strong>Güzel Teknoloji</strong>
</p>

⭐ Bu projeyi beğendiyseniz yıldız vermeyi unutmayın! / If you liked this project, don't forget to star it!

</div>
